 ...
 * @brief TRT_POSE library C++ porting source file.
 */
    ...
    const float threshold, const int window_size, const int max_count)
{
    ...
    const int N = input.size(0);
    const int C = input.size(1);
    const int H = input.size(2); (void) H;
    const int W = input.size(3); (void) W;
    const int M = max_count;
    ...
    // find peaks
    find_peaks_out_torch(counts, peaks, input, threshold, window_size, 
        max_count);
    ...
    torch::Tensor peaks, torch::Tensor cmap, int window_size)
{
    ...
    refine_peaks_out_nchw(
        ...
        (const int *)peaks.data_ptr(), (const float *)cmap.data_ptr(), 
        N, C, H, W, M, window_size);
}
    ...
    torch::Tensor topology, torch::Tensor counts, const float score_threshold)
{
    ...
    assignment_out_nk(
        ...
        (const int *) topology.data_ptr(), (const int *) counts.data_ptr(), 
        N, C, K, M, score_threshold, workspace);
    ...
    torch::Tensor topology, torch::Tensor counts, int max_count)
{
    ...
    int N = counts.size(0);
    int K = topology.size(0); (void) K;
    int C = counts.size(1);
    int M = connections.size(3); (void) M;

    auto objects = torch::full({N, max_count, C}, -1, options);
    auto object_counts = torch::zeros({N}, options);
    connect_parts_out_torch(object_counts, objects, connections, topology, 
        counts, max_count);
